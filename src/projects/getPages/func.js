...
/**
 * 下拉框的数据 * *
 * @param {any} [$=window.jQuery]
 * @returns
 */

function getComboboxData($ = window.jQuery) {
...
function writeFile($ = window.$ || window.jQuery) {
  ...
  /* 综合查询 or 统计分析 */
  // var $panel = $(".panel:not(#panel_home)");
  /* title */
  // let title = $panel.find(".panel-title:contains('当前位置：孕产妇保健')").text().split(">");
  ...
  //     action: "writeFile",
  //     content: {
  //         contents: getModelDslRuleFromPanels(".panel:not(#panel_home) .panel .easyui-panel.panel-body"),
  //         // contents: getModelDslRuleFromPanels(".panel.theme-panel-blue.easyui-fluid"),
  //         filename: `输入表单-儿童保健-统计分析-${title}.js`
  //     },
  // });

  /* 综合统计 */
  // function getTableHeaderZHTJ(selector, $ = window.$) {
  //     let $table = $(selector);
  //     let tableHeader = [];
  ...
  //     content: {
  //         contents: (() => {})(),
  //         filename: `列表表头-孕产妇保健-统计分析-${title}.js`
  //     }
  // });
  ...
  //     action: "writeFile",
  //     content: {
  //         contents: getTableHeaderZHTJ(`#maintable`),
  //         filename: `列表表头-${$("#exec-payload").val()}.js`
  //     },
  // });
  ...
  postM({
    action: "writeFile",
    content: {
      contents: getModelDslRuleFromPanels("#panel_HYUTE23PEJ"),
      // contents: getModelDslRuleFromPanels(".panel.theme-panel-blue.easyui-fluid"),
      filename: `儿童保健-输入表单-${$("#exec-payload").val()}.js`,
    },
  });
  ...
  postM({
    action: "writeFile",
    content: {
      contents: getTableHeader(`#maintable`),
      filename: `儿童保健-列表表头-${$("#exec-payload").val()}.js`,
    },
  });
  ...
  //     action: "writeFile",
  //     content: {
  //         contents: getTableHeader(`#maintable1`),
  //         filename: `列表表头-${$("#exec-payload").val()}1.js`
  //     },
  // });
  // postM({
  //     action: "writeFile",
  //     content: {
  //         contents: getTableHeader(`#maintable2`),
  //         filename: `列表表头-${$("#exec-payload").val()}2.js`
  //     },
  // });
  // postM({
  //     action: "writeFile",
  //     content: {
  //         contents: getTableHeader(`#maintable3`),
  //         filename: `列表表头-${$("#exec-payload").val()}3.js`
  //     },
  // });
  // postM({
  //     action: "writeFile",
  //     content: {
  //         contents: getTableHeader(`#maintable4`),
  //         filename: `列表表头-${$("#exec-payload").val()}4.js`
  //     },
  // });
  ...
  /* 
      ...
      postM({
           action: "writeFile",
           content: {
               contents: getModelDslRuleFromPanels("#maintable"),
  ...
  /*  debugger;
     postM({
         ...
         content: {
             ...
             // contents: consoleDataFromTable(".panel.theme-panel-blue.easyui-fluid"),
             filename: `列表查询表单-${$("#exec-payload").val()}.js`
         },
     }); */
  /**
   * 列表表头
   */
  // let $targetArray = $("[data-cc]");
  // let targetArray = [];

  // Array.prototype.map.call($targetArray, target => {
  //     targetArray.push({
  //         text: target.innerHTML,
  //         cc: target.dataset.cc,
  //         lx: target.dataset.lx
  //     });
  // });
  ...
  //             filename: `孕产妇保健-孕期保健-档案首页-${element.cc}.js`
  //         },
  //     });
  // }
  ...
  //     action: "writeFile",
  //     content: {
  //         contents: targetArray,
  //         filename: `孕产妇保健-孕期保健-档案首页.menu.js`
  //     },
  // });
}
...
/*  */
function getModelDslRuleFromPanels(selector, $ = window.$) {
  let $panels = $(selector);
  let console_log = "";
  let model = {},
    rules = {},
...
function getModelFrom($panels, $ = window.$) {
  ...
  Array.prototype.map.call($items4modelValue, (items4modelValueDom) => {
    ...
    items4modelValueObj[$items4modelValueDom.attr("id")] =
      $items4modelValueDom.attr("value");
  });
  /*  */
  let model = {};
  ...
  Array.prototype.map.call($trS, (trDom) => {
    ...
    Array.prototype.map.call($tdS, (tdDom) => {
      ...
      Array.prototype.map.call($items, (itemDom) => {
        ...
        (() => {
          console.log("itemDom.type", itemDom.type);
          ...
          if (itemDom.classList.contains("switchbutton-f")) {
            ...
            item.label = "无label";
            try {
              item.label = $td.find(".input-label")[0] || item.label.innerText;
            } catch (error) {
              console.log(error);
            }
    ...
    Array.prototype.map.call($trS, (trDom) => {
      ...
      Array.prototype.map.call($tdS, (tdDom) => {
        ...
        Array.prototype.map.call($items, (itemDom) => {
          ...
          /*  类型：scha-vue\src\components\input_type_string.js*/
          (() => {
            console.log(itemDom.type);
            if (itemDom.type === "radio") {
              item.type = "radio";
              return;
            }
            if (itemDom.classList.contains("datebox-f")) {
            ...
            if (itemDom.classList.contains("switchbutton-f")) {
              ...
              try {
                item.label =
                  $td.find(".input-label")[0] || item.label.innerText;
              } catch (error) {
                console.log(error);
              }
        ...
        Array.prototype.map.call($items2, (itemDom) => {
          let item = {
            prop: "prop:id",
            type: "input",
            placeholder: "placeholder",
...
function converItem($subItem, subItem, $ = window.$) {
  ...
  let optionString = $subItem.data().options || $subItem.data().name;
        ...
        if (regexMatch) {
          if (
            ...
            options[property] = Boolean(
              "true" === (regexMatch[1] || regexMatch[2])
            );
          } else {
